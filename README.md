...
son visage, des outils qui lui font pousser des bras, des livres sur son
torse. La silhouette du robot dit d'un coup d'œil ce que l'agent sait
faire — puis on discute avec lui, ou on exporte sa configuration en JSON.

![L'interface complète : le robot équipé, la conversation, les réglages](docs/interface.png)

Chaque objet posé a une conséquence visible, et une contrepartie réelle
dans l'appel au modèle :

| Objet | Où il va | Ce que ça change |
|---|---|---|
| 🧠 **Cerveau** | sa tête | le modèle réellement appelé, écrit sur le cerveau |
| 👀 **Skill** | son visage | une paire d'yeux de plus, une capacité déclarée au modèle |
| 🔨 **Outil** | ses épaules | **un bras pousse** et un poing se referme sur l'outil |
| 📚 **RAG** | son torse | les connaissances s'empilent sur sa poitrine |
...
- **Outils réels** : le JSON des outils existe déjà ; reste à l'envoyer au
